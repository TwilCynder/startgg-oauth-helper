...
- **initCallbackEndpoint(server, path, client_id, client_secret, redirect_uri, scopes, callback, responseHandlerCallback)** : initializes the oauth "**callback**" endpoint, which receives the redirect from start.gg's page, and handles obtaining the API token, then saves it using express-session. 
    ...
    - The token, as well as the Oauth refresh token, and API token expiration date, are passed to the `responseHandlerCallback(req, responseBody)` ; this callback can be omitted in favor of the default behavior (which you want in most cases) which is as follows : the token is saved using express-session under the `startgg` property. This means that after the token is first obtained, and as long as `express-session`'s middleware is in use, for all subsequest requests (to any route) from the same client, `req.session.startgg` will have the following value :
        ```js
        {
            access_token: "token",
...
- The `paths` parameter must be an object containing the path for each route.
    ...
    ```  
    Omitting the path for an optional route will disable this route, with the exception of the `callback` route which is always enabled ; however that path can also be omitted, as it can be inferred from the Redirect URI (as explained above)
- The `config` parameter must be an object and may contain additional options : 
  - `state` : passed as the `state` argument to the "auth redirect" route init function
  - `finalCallback` passed as the `callback` argument to the "callback" route init function
  - `responseHandlerCallback` passed as the `responseHandlerCallback` argument to the "callback" and "token" init functions
  - `startggDataGetter` passed as the `startggDataGetter` argument to the "token" init function
